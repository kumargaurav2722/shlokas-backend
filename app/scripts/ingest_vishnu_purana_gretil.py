...
REF_RE = re.compile(r"ViP_(\d+),(\d+)\.(\d+)")
TRAILING_REF_RE = re.compile(r"\s*//\s*ViP_\d+,\d+\.\d+(?:\*\d+)?\s*//\s*")
SKIP_TOKENS = (
    "ins.",
    "subst.",
    "var.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest Vishnu Purana from GRETIL plain text HTML")
    parser.add_argument(
        "--url",
        default="https://gretil.sub.uni-goettingen.de/gretil/corpustei/transformations/html/sa_viSNupurANa.htm",
    )
    parser.add_argument("--category", default="purana")
    parser.add_argument("--work", default="Vishnu Purana")
